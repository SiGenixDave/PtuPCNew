 ...
 *                                  1.  R188 modifications resulting from documents: 'PTU MOC Findings - .docx' and 'PTU Installation on 64-bit Machine_v1-08022015.docx'
 *                                      sent from Atul Chaudhari on 4th Aug 2015 and the follow up email sent on 5th Aug 2015.
 *                                      
...
#region - [Revision History 6.14] -
/*
 *  08/26/15    6.14    K.McD       References
 ...
 *                                      appropriate configuration file is not already present on the PTU computer.
 *                                      
 *                                  2.  Bug Fix - SNCR - R188 [20-Mar215] Item 29. The file app.config associated with the Event sub-system had not been added
 *                                      to the TortoiseSVN repository, consequently, when an attempt is made to build the files that have been checked out of the
 *                                      repository a number of errors are generated.
 *                                      
 *                                  3.  'Build/Configuration Manager' changes to remove any unnecessary build options.
 *                                  
 *                                  4.  'Tools/Options...' changes to set the settings to the default C# settings with the following modifications:
 *                                       1.  'Text Editor/All Languages/General' - Word Wrap: yes, Show visual glyphs for word wrap: yes.
 *                                       2.  'Text Editor/All Languages/Tabs' - Indenting: Smart, Tabs - Tab size: 4, Indent size: 4, Insert Spaces.
 *                                      
 *                                  Modifications
 *                                  1.  Modified the Configuration Manager options to only support 'Any CPU', 'Win32' and 'x64'. - Ref.: 3.
 *                                  2.  Updated the 'Exported VS2010 Settimgs.vssettings' file to allow new development systems to be easily updated with the
 *                                      latest modifications to 'Tools/Options...'. - Ref.: 4.
 *                                  3.  Modified Resources.resx. Renamed the resource 'FilenameConfigDownload' to 'FilenameCommandFileFTPDownload' and updated the
 *                                      Windows command filename to 'FTP Transfer.cmd'. Also created the resources: (a) MBTConfigVCUDownloadCommandFileNotFound, to
 *                                      report the message that the Windows command file cannot be found, and (b) MBTConfigVCUDownloadFailedWithErrorCode to inform
 *                                      the user of the error code that was returned from the command file if it is not successful.
 *                                  4.  MdiPTU.Support.cs. Rev. 1.22. - Ref. 1.
 *                                      
 *                                  Dynamic Link Library Updates
 *                                  1.  Common Application Extension Assembly. Rev. 1.25. - Ref.: 1.
 *                                  2.  Event Application Extension Assembly. Rev.1.24. Ref.: 2.
 */
#endregion - [Revision History 6.14] -

#region - [Revision History 6.15.1] -
/*
 *  09/30/15    6.15.1  K.McD       References
 ...
 *                                      
 *                                  2.  Part 2 of the upgrade to the Chicago 5000 PTU software that allows the user to download the configuration and help files for
 *                                      a particular Chicago 5000 vehicle control unit (VCU) via an ethernet connection to the FTP (File Transfer Protocol) server
 *                                      running on the VCU. The scope of Part 2 of the upgrade is defined in purchase order  4800011369-CU2 28.07.2015.
 *                                      
 *                                  Modifications
 *                                  1.  Modified ControlPanel.cs. Rev. 1.3, ControlPanel.Designer.cs. Rev. 1.3 - Ref.1.
 *                                  2.  Modified MdiPTU.cs. Rev. 1.19. - Ref.: 2.
 *                                  3.  Added FtpErrorProcessing.cs. Rev. 1.0. - Ref.: 2.
 *                                  4.  Modified 'FTP Transfer.cmd' and renamed it to 'FtpTransfer.cmd'. Rev. 2.0. - Ref.: 2.
 *                                  5.  Modified Resources.resx. - Ref.: 2.
 *                                      1.  Added FTP Error Codes.
 *                                      2.  Added Windows Command File Arguments for each project.
 *                                      3.  Modified the name and string associated with resource MBTConfigVCUDownloadUnsuccessfulWithErrorCode. 
 *                                      4.  Modified the MBTConfigVCUDownloadStart resource string to add the instruction '3. Move the mouse to the 'virtual machine' down
 *                                          arrow box at the top of the screen. Click the down arrow box once and select 'removable devices'. Click on 'network adapter'
 *                                          and ensure that the text indicates 'disconnect'. If true, no further action is required.  If it indicates 'connect', then click
 *                                          'connect' and the network adapter will be enabled.'.
 *                                      5.  Changed the name of the FilenameCommandFileFTPDownload resource to FilenameWindowsCommandFile and changed the corresponding
 *                                          text to 'FtpTransfer.cmd'.
 *                                  
 *                                  Dynamic Link Library Updates
 *                                  1.  Common Application Extension Assembly. Rev. 1.26. - Ref.: 1.
 *                                  2.  Event Application Extension Assembly. Rev.1.25. Ref.: 1.
 *  
 */
